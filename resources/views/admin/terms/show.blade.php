...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between btn-sm">
            ...
            <a href="{{ isset($term) ? route('terms.edit', $term->id) : route('terms.create') }}"
                class="btn btn-dark">{{ isset($term) ? 'Edit Terms' : 'Add Terms' }}</a>
        </div>

        <div class="card-body">
            @if (isset($term))
                <div class="form-group">
                    <input type="text" class="form-control" placeholder="eg Tercera Terms & Conditions"
                        value="{!! $term->heading !!}" readonly="">
                </div>
                <div>{!! $term->terms !!}</div>
            @else
