...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between btn-sm">
            ...
            <a href="{{ isset($about) ? route('about.edit', $about->id) : route('about.create') }}"
                class="btn btn-dark">{{ isset($about) ? 'Edit about' : 'Add about' }}</a>
        </div>

        <div class="card-body">
            @if (isset($about))
                <div class="form-group">
                    <input type="text" class="form-control" placeholder="eg Tercera abouts & Conditions"
                        value="{!! $about->heading !!}" readonly="">
                </div>
                <div>{!! $about->about !!}</div>
            @else
